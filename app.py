...
def estimate_token_usage(json_data):
...
def render_backend_logs_visuals(log_txt):
    ...
    if sim_matches:
        ...
        df_viz_sorted = df_viz.sort_values('Score', ascending=True).reset_index(drop=True)
        min_score = df_viz_sorted['Score'].min()
        max_score = df_viz_sorted['Score'].max()
        y_min = max(0, min_score - (max_score - min_score) * 0.3) if max_score != min_score else 0
        y_max = max_score + (max_score - min_score) * 0.3 if max_score != min_score else max_score + 0.01
        fig = px.scatter(
            df_viz_sorted, x=df_viz_sorted.index, y='Score',
            hover_data={'Score': ':.4f', 'FullText': True},
            labels={'Score': '相似度得分', 'index': ''}
        )
        fig.update_layout(
            yaxis=dict(range=[y_min, y_max], showgrid=False, title='相似度得分'),
            xaxis=dict(
            ...
            height=350,
            width=max(400, len(df_viz_sorted) * 80),
            margin=dict(l=60, r=10, t=40, b=20),
            title={'text': "Top K 相似样本匹配度分析", 'y': 0.98, 'x': 0.5, 'xanchor': 'center', 'yanchor': 'top'}
        )
        fig.update_traces(
...
def main():
    # --- 侧边栏：全局配置 ---
    st.sidebar.header("全局资源配置")

    if "json_data" in st.session_state:
        in_tokens, out_tokens = estimate_token_usage(st.session_state.json_data)
        st.sidebar.markdown("### Token 用量预估")
        ...
        st.sidebar.markdown(
            ...
            f"<span style='font-size:12px; color:gray;'>本次批处理预计总消耗</span><br>"
            f"<span style='font-size:20px; font-weight:bold; color:#5b9bd5;'>{total_tokens / 1000:.1f}k Tokens</span>"
            f"</div>", unsafe_allow_html=True
        )
    else:
        st.sidebar.info("请先加载 JSON 文件以查看用量预估")

    st.sidebar.divider()

    st.sidebar.markdown("### 核心算法参数")
    ngram_n = st.sidebar.slider(
        "N-Gram 相似度窗口", min_value=1, max_value=20, value=4,
        help="用于在知识库中匹配相似样本的 N-gram 字符窗口大小。值越大，匹配越严格。"
    )
    apply_ngram_patch(ngram_n)

    dyn_ex_count = st.sidebar.slider(
        "动态示例数量", min_value=1, max_value=10, value=6,
    ...
    model = st.sidebar.selectbox("推理模型选择", model_options, index=new_model_index, key="model_selector")
    ...
    with tab2:
        if "json_data" not in st.session_state:
            ...
            with c2:
                ...
                with col_btn_run:
                    run_single = st.button("启动单条处理", type="primary", use_container_width=True)
                with col_btn_stop:
                ...
                if run_single:
                    if "train_data" not in st.session_state:
                        ...
                        if hasattr(llm_select_topk, 'TRAIN_EXAMPLES'): llm_select_topk.TRAIN_EXAMPLES = []
                        if hasattr(onlyselect, 'TRAIN_EXAMPLES'): onlyselect.TRAIN_EXAMPLES = []
                    else:
    ...
    with tab3:
        if "json_data" not in st.session_state:
            ...
            with col_cfg2:
                ...
                selected_tag = st.selectbox("数据集溯源标签 (Dataset)", dataset_options, index=default_index)
                if selected_tag == "自定义...":
            ...
            with c_s:
                start_btn = st.button("集群运算", type="primary", disabled=(ckpt_path is None))
            with c_e:
            ...
            if start_btn:
                ...
                bar = st.progress(start / total if total > 0 else 0, text="运行中...")
                log_box = st.empty()
                ...
                for i in range(start, total):
                    ...
                    if processed > 0:
                        ...
                        bar.progress((i + 1) / total,
                                     text=f"系统进度: {i + 1}/{total} | TTA: {remain / 60:.1f} 分钟")
    ...
    with tab4:
            ...
            if df_hist.empty:
                ...
                for ds in all_datasets:
                    ...
                    with c_filter1:
                        ...
                        sel_methods = st.multiselect(f"筛选运行模式 ({ds})", options=methods, default=methods,
                                                     key=f"method_{ds}")
                    with c_filter2:
                        ngrams = sorted(df_ds['N-Gram'].unique())
                        sel_ngrams = st.multiselect(f"筛选N-Gram值 ({ds})", options=ngrams, default=ngrams,
                                                    key=f"ngram_{ds}")

                    filtered_df = df_ds[
                        (df_ds['Method'].isin(sel_methods)) & (df_ds['N-Gram'].isin(sel_ngrams))]
                    ...
                    fig_line = px.line(
                        trend_df, x="Time", y="F1_val", color="Method", markers=True,
                        hover_data=["Filename", "N-Gram"],
                        labels={"F1_val": "F1 Score (%)", "Method": "运行模式"}, height=400
                    )
        ...
        if os.path.exists(HISTORY_FILE) and 'df_hist' in locals() and not df_hist.empty:
            with st.expander("数据修改", expanded=False):
                record_list = df_hist.apply(
                    lambda row: f"[{row['Dataset']}] {row['Filename']} ({row['Time']})", axis=1).tolist()
                ...
                if selected_record_str:
                    ...
                    new_dataset_tag = st.text_input("重置数据", value=target_row['Dataset'])
                ...
                def render_case(case_data):
                    ...
                    with col_p:
                        ...
                        for fp_item, fn_item in paired_matches.items():
                            ...
                            label = f"{errors[0]}错误" if len(errors) == 1 else "混合错误"
